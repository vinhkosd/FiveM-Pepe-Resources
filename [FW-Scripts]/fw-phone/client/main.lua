        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    Framework.Functions.Notify("Hành động không thể thực hiện..", "error")
                end
            end
        end
        Citizen.Wait(3)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Anh bạn à, sao lại nhắn tin cho chính mình vậy, anh bạn ổn chứ?",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
            ...
            SendNUIMessage({
                ...
                NotifyData = {
                    title = "Whatsapp", 
                    content = "Bạn đã nhận được một tin nhắn mới từ "..IsNumberInContacts(SenderNumber).."!", 
                    icon = "fab fa-whatsapp", 
                    timeout = 3500, 
                    color = "#25D366",
                },
...
AddEventHandler('fw-phone:client:NewMailNotify', function(MailData)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Mail",
                text = "Bạn đã nhận được một Email mới từ "..MailData.sender,
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Mail", 
                content = "Bạn đã nhận được một Email mới từ "..MailData.sender, 
                icon = "fas fa-envelope", 
                timeout = 3500, 
                color = "#ff002f",
            },
...
AddEventHandler('fw-phone:client:send:email:bought:vehicle', function(Plate)
    ...
    SetTimeout(math.random(4500, 8000), function()
        TriggerServerEvent('fw-phone:server:sendNewMail', {
            sender = "Autoscout24",
            subject = "Bạn đã mua một chiếc xe mới!",
            message = "Chào " .. Player.charinfo.firstname .. " " .. Player.charinfo.lastname .. ",<br /><br />Bạn đã mua một chiếc xe mới <br><br>Biển số: <b>" .. Plate .. "</b> <br><br>Chúc quý khách vạn dặm bình an!<br /><br />Trân trọng,<br />Autoscout24",
            button = {}
        })
    end)
end)
...
AddEventHandler('fw-phone:client:send:email:sold:vehicle', function(SellPrice, VehiclePlate)
    SetTimeout(math.random(4500, 8000), function()
        TriggerServerEvent('fw-phone:server:sendNewMail', {
            sender = "Autoscout24",
            subject = "Xe của bạn đã bán",
            message = "Chào ,<br/><br/>Bạn nhận được e-mail về đơn hàng gần đây.<br><br>Biển số: <strong>" ..VehiclePlate.. "</strong> <br>Giá bán: <strong>€"..SellPrice.. '</strong><br><br>Xe đã được bán thành công và số tiền đã được gửi vào ngân hàng cho bạn.<br><br>Trân trọng,<br>Autoscout24',
            button = {}
        })
    end)
end)
...
AddEventHandler('fw-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Quảng cáo",
                text = "Một tin mới đã được đăng bởi "..LastAd,
                icon = "fas fa-ad",
                color = "#ff8f1a",
                timeout = 2500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Quảng cáo", 
                content = "Một tin mới đã được đăng bởi "..LastAd,
                icon = "fas fa-ad", 
                timeout = 2500, 
                color = "#ff8f1a",
            },
                ...
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    ...
                    SetTimeout(2500, function()
                        SendNUIMessage({
                            ...
                            PhoneNotify = {
                                title = "Twitter", 
                                text = "Không thể nhắc đến bản thân!", 
                                icon = "fab fa-twitter",
                                color = "#1DA1F2",
                            },
                        })
...
AddEventHandler('fw-phone:client:NotEnoughBits', function(amount, newmoney)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Twitter", 
            text = "Không đủ Bits để đăng tweet ẩn danh", 
            icon = "fab fa-twitter",
            color = "#1DA1F2",
        },
    })
...
AddEventHandler('fw-phone:client:TransferMoney', function(amount, newmoney)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Bank", text = "Bạn nhận được €"..amount.."!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Bank", content = "Bạn nhận được €"..amount.."!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
    ...
    if PhoneData ~= nil and PhoneData.PlayerData ~= nil then
      local MyPlayerId = PhoneData.PlayerData.source
       if src ~= MyPlayerId then
          local titel = "Tweet mới (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")"
              if not PhoneData.isOpen then
                  SendNUIMessage({
                      action = "Notification",
                      NotifyData = {
...
AddEventHandler('fw-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Bạn đã được nhắc đến trong một Tweet!", content = TweetMessage.message, icon = "fab fa-twitter", timeout = 3500, color = nil, }, })
    else
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Bạn đã được nhắc đến trong một Tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
    end
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    Framework.Functions.Notify('Vị trí GPS được đặt: '..data.alert.title)
        ...
        if v.name == Name and v.number == Number then
            ...
            if PhoneData.isOpen then
                ...
                SendNUIMessage({
                    ...
                    NotifyData = {
                        title = "Telefoon", 
                        content = "Bạn đã xóa một liên hệ!", 
                        icon = "fa fa-phone-alt", 
                        timeout = 3500, 
                        color = "#04b543",
                    },
...
AddEventHandler('fw-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Điện thoại",
                text = "Đã thêm liên hệ vào mục gợi ý!", 
                icon = "fa fa-phone-alt",
                color = "#04b543",
                timeout = 1500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Điện thoại", 
                content = "Đã thêm liên hệ vào mục gợi ý!", 
                icon = "fa fa-phone-alt", 
                timeout = 3500, 
                color = "#04b543",
            },
...
AddEventHandler('fw-phone:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = "Bạn bị trừ €"..amount.." trong tài khoản thẻ!", 
                icon = "fas fa-university", 
                color = "#ff002f",
                timeout = 3500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Bank",
                content = "Bạn bị trừ €"..amount.." trong tài khoản thẻ!", 
                icon = "fas fa-university",
                timeout = 3500, 
                color = "#ff002f",
            },
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    Framework.Functions.Notify("Vị trí GPS đã được đặt: " .. data.HouseData.HouseData['Adres'] .. "!", "info")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    Framework.Functions.Notify('Vị trí GPS đã được đặt!', 'success')
end)
