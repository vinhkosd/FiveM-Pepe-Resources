...
$(document).on('click', '.settings-app-tab', function(e){
    ...
    if (PressedTab == "background") {
        ...
        if (!QB.Phone.Data.AnonymousCall) {
            $("#numberrecognition > p").html('Chưa chọn');
        } else {
            $("#numberrecognition > p").html('Đã chọn');
        }
    }
});
...
$(document).on('click', '#accept-background', function(e){
    ...
    if (hasCustomBackground === false) {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Cài đặt", QB.Phone.Settings.Backgrounds[QB.Phone.Settings.Background].label+" được cài đặt!")
        ...
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Cài đặt", "Ảnh nền tùy chỉnh đã được cài đặt!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $(".phone-background").css({"background-image":"url('"+QB.Phone.Settings.Background+"')"});
    }

    $.post('http://fw-phone/SetBackground', JSON.stringify({
...
$(document).on('click', '#accept-profilepicture', function(e){
    ...
    if (ProfilePicture === "default") {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Cài đặt", "Ảnh hồ sơ mặc định!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="./img/default.png">');
    } else {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Cài đặt", "Ảnh hồ sơ cá nhân đã được đặt!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        console.log(ProfilePicture)
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="'+ProfilePicture+'">');
    }
    $.post('http://fw-phone/UpdateProfilePicture', JSON.stringify({
