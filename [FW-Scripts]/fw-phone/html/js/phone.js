...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://fw-phone/ClearRecentAlerts');
                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Bạn đã bắt đầu một cuộc gọi ẩn danh!");
                        }
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Bạn đang có một cuộc gọi khác!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Thuê bao bận!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Số điện thoại không hợp lệ hoặc người chơi không online!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Bạn không thể gọi số của mình!");
        }
    });
});
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Bạn đang có cuộc gọi khác!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Thuê bao bận!");
                }
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        ...
        $("#total-contacts").text(TotalContacts+ " liên hệ");
    } else {
        $("#total-contacts").text("0 liên hệ #SAD");
    }
};
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        $.post('http://fw-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
        ...
        setTimeout(function(){
            ...
            $.post('http://fw-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Điện thoại", "Bạn không thể nhắn tin cho bản thân..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://fw-phone/EditContact', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Sửa liên hệ", "Vui lòng điền đầy đủ thông tin!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://fw-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://fw-phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://fw-phone/RemoveSuggestion', JSON.stringify({
            ...
            $(".amount-of-suggested-contacts").html(amount + " liên hệ");
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Thêm liên hệ", "Vui lòng điền đầy đủ thông tin!");
    }
});
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Bạn đang có 1 cuộc gọi khác!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Máy bận!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Thuê bao quý khách vừa gọi tạm thời không liên lạc được!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Điện thoại", "Bạn không thể gọi cho chính mình!");
        }
    });
}
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://fw-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://fw-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://fw-phone/CancelOngoingCall');
});
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Bạn có một cuộc gọi đến từ "+CallData.name
                if (AnonymousCall) {
                    Label = "Bạn đang được gọi bằng một số ẩn danh"
                }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            $(".phone-currentcall-title").html("Cuộc gọi đến");
        } else if (cData.CallType == "outgoing") {
            $(".phone-currentcall-title").html("Cuộc gọi đi");
        } else if (cData.CallType == "ongoing") {
            $(".phone-currentcall-title").html("Đang gọi ("+cData.CallTime+")");
        }
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        $(".amount-of-suggested-contacts").html(AmountOfSuggestions + " liên hệ");
        ...
        $(".amount-of-suggested-contacts").html("0 liên hệ");
    }
}
