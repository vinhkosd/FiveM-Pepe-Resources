...
RegisterNetEvent('fw-vehiclefailure:client:fix:veh')
AddEventHandler('fw-vehiclefailure:client:fix:veh', function()
	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		...
		SetVehicleFuelLevel(Vehicle, Amount + 0.0)
 		TriggerServerEvent('fw-fuel:server:update:fuel', vehiclePlate, Vehicle, math.floor(Amount))
	else
		local Vehicle = Framework.Functions.GetClosestVehicle()
		SetVehicleFixed(Vehicle)
	end
	Framework.Functions.Notify("Voertuig gefixt nerd", 'success')
end)