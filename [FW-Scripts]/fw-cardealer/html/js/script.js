...
$(document).on('click', '.car-slot', function(e) {
    ...
    $.post('http://fw-cardealer/Click', JSON.stringify({}))
...
$(document).on('click', '.subit-button', function(e) {
    ...
    $.post('http://fw-cardealer/Click', JSON.stringify({}))
    $.post('http://fw-cardealer/SetSlotVehicle', JSON.stringify({slot: CurrentData.Slot, model: SelectedModel}))
...
$(document).on('click', '.menu-item', function(e) {
    ...
    if (MenuType != CurrentMenu) {
      ...
      $.post('http://fw-cardealer/ErrorSound', JSON.stringify({}))
    }
});

SetupStock = function() {
  $.post('http://fw-cardealer/GetStockVehicles', JSON.stringify({}), function(CarData){
...
SetupCarSlots = function() {
  $.post('http://fw-cardealer/GetDisplayVehicles', JSON.stringify({}), function(CarData){
    for (const [key, value] of Object.entries(CarData)) {
...
CloseTablet = function() {
  $('.main-container').animate({"left": "-180vh"}, 250, function() {
    ...
    $.post('http://fw-cardealer/CloseNui', JSON.stringify({}))
  })
}
