...
RegisterNetEvent('fw-cardealer:client:sync:premium:dealer')
AddEventHandler('fw-cardealer:client:sync:premium:dealer', function(Slot, ConfigData)
    Config.PremiumDealerSpots[Slot] = ConfigData
    RefreshCars()
end)

RegisterNetEvent('fw-cardealer:client:set:premium:details')
AddEventHandler('fw-cardealer:client:set:premium:details', function(Vehicle, Price, Display, Stock)
    Config.PremiumVehicleDetails[Vehicle] = {['Price'] = Price, ['Display'] = Display, ['Stock'] = Stock}
end)

RegisterNetEvent('fw-cardealer:client:can:buy:vehicle')
AddEventHandler('fw-cardealer:client:can:buy:vehicle', function(VehicleSlot)
    CurrentBuyVehicle = VehicleSlot
end)

RegisterNetEvent('fw-cardealer:client:set:stock')
AddEventHandler('fw-cardealer:client:set:stock', function(StockData, Slot)
    Config.PremiumDealerSpots[Slot]['Stock'] = StockData
end)

RegisterNetEvent('fw-cardealer:client:spawn:car:premium')
AddEventHandler('fw-cardealer:client:spawn:car:premium', function(VehicleName, Plate)
...
RegisterNetEvent('fw-cardealer:client:sell:closest:vehicle')
AddEventHandler('fw-cardealer:client:sell:closest:vehicle', function()
    if CurrentVehicle ~= nil then
        ...
        if Player ~= -1 and Distance < 2.5 then
            Framework.Functions.Progressbar("lockpick-door", "Voertuig Verkopen..", 5500, false, false, {
                ...
                TriggerServerEvent('fw-cardealer:server:sell:closest', GetPlayerServerId(Player), CurrentVehicle)
...
RegisterNUICallback('SetCarInSlot', function(data)
    TriggerServerEvent('fw-cardealer:server:set:premium:data', data.slot, data.price, data.model, data.stock)
end)
                ...
                if CurrentBuyVehicle ~= nil and NearCar then
                    ...
                    if Distance <= 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent('fw-cardealer:server:buy:current:vehicle', CurrentBuyVehicle)
...
RegisterNUICallback('SetSlotVehicle', function(data)
    if Config.PremiumVehicleDetails[data.model] ~= nil then
        TriggerServerEvent('fw-cardealer:server:set:premium:data', data.slot, Config.PremiumVehicleDetails[data.model]['Price'], data.model, Config.PremiumVehicleDetails[data.model]['Display'], Config.PremiumVehicleDetails[data.model]['Stock'])
    end
end)
