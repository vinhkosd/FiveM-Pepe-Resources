        ...
        if PlayerData.job.name == "police" then
            ...
            if (IsPedInAnyVehicle( ped ) ) then 
                ...
                if (IsPoliceVehicle(vehicle)) then
                    ...
                    Framework.Functions.Notify( "Je moet in een politie voertuig zitten!", "error")
                end 
            else 
                Framework.Functions.Notify( "Je moet in een voertuig zitten!", "error")
            end 
        end
    end)
end)
...
AddEventHandler( 'wk:changeRadarLimit', function( speed ) 
    ...
    Framework.Functions.Notify("Max snelheid gezet op: " .. speed .. "km/u")
end )
                ...
                if ( radarInfo.fwdXmit ) then  
                    ...
                    if ( DoesEntityExist( fwdVeh ) and IsEntityAVehicle( fwdVeh ) ) then 
                        ...
                        if plateChecked ~= fwdPlate then
                            Framework.Functions.TriggerCallback('police:IsPlateFlagged', function(result)
                                if result then
                                    ...
                                    radarInfo.fwdFastLocked = true 
                                    SendNUIMessage( { lockfwdfast = true } )
                                    Framework.Functions.Notify("Gemarkeerd voertuig gevonden!", "error")
                                end
...
Citizen.CreateThread( function()
    while true do 
        ...
        if Framework ~= nil then 
            ...
            if ( IsDisabledControlPressed( 0, Keys["LEFTSHIFT"] ) and IsDisabledControlJustPressed( 0, Keys["7"] ) ) then 
            ...
            if ( not IsDisabledControlPressed( 0, Keys["LEFTSHIFT"] ) and IsDisabledControlJustPressed( 0, Keys["7"] ) ) then 
                if Framework.Functions.GetPlayerData().job.name == "police" and Framework.Functions.GetPlayerData().job.onduty then
                    ResetFrontFast()
                    ResetRearFast()
                    ResetFrontAntenna()
                    ResetRearAntenna()
                    Framework.Functions.Notify("Politie ANPR gereset")
                end
            end 
            ...
            if ( IsDisabledControlJustPressed( 0, Keys["8"] ) ) then 
                if Framework.Functions.GetPlayerData().job.name == "police" and Framework.Functions.GetPlayerData().job.onduty then
                    ...
                    if (radarInfo.plateLocked) then
                        ...
                        Framework.Functions.Notify("Kenteken locked: " .. radarInfo.fwdPlate)
                    else
                        SendNUIMessage( { unlockPlate = true } )
                        Framework.Functions.Notify("Kenteken unlocked")
                    end
                end
            end
