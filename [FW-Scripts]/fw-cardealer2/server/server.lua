...
Citizen.CreateThread(function()
    Citizen.SetTimeout(1000, function()
        Framework.Functions.ExecuteSql(false, "SELECT * FROM `server-cars` WHERE `type` = 'second'", function(result)
            for k, v in pairs(result) do
                Config.PremiumVehicleDetails[v.vehiclename] = {['Price'] = v.price, ['Display'] = v.vehicledisplayname, ['Stock'] = v.stock}
                TriggerClientEvent('fw-cardealer2:client:set:premium:details', -1, v.vehiclename, v.price, v.vehicledisplayname, v.stock)
                Citizen.Wait(3)
            end
        end)
    end)
...
Framework.Functions.CreateCallback("fw-cardealer2:server:get:config", function(source, cb)
    cb(Config)
end)

Framework.Functions.CreateCallback("fw-cardealer2:server:get:storage:cars", function(source, cb)
    local CarsTable = {}
    Framework.Functions.ExecuteSql(true, "SELECT * FROM `server-cars` WHERE `type` = 'second'", function(result)
        for k, v in pairs(result) do
            local TempTable = {}
            TempTable = {
...
RegisterServerEvent('fw-cardealer2:server:set:premium:data')
AddEventHandler('fw-cardealer2:server:set:premium:data', function(Slot, Price, Model, DisplayName, Stock)
 ...
 TriggerClientEvent('fw-cardealer2:client:sync:premium:dealer', -1, Slot, Config.PremiumDealerSpots[Slot])
end)

RegisterServerEvent('fw-cardealer2:server:sell:closest')
AddEventHandler('fw-cardealer2:server:sell:closest', function(TargetPlayer, CurrentSlot)
    local TargetSource = Framework.Functions.GetPlayer(TargetPlayer)
    if TargetSource ~= nil then
        TriggerClientEvent('fw-cardealer2:client:can:buy:vehicle', TargetSource.PlayerData.source, CurrentSlot)
    end
end)

RegisterServerEvent('fw-cardealer2:server:buy:current:vehicle')
AddEventHandler('fw-cardealer2:server:buy:current:vehicle', function(CurrentSlot)
    local src = source
    ...
    if Config.PremiumDealerSpots[CurrentSlot]['Stock'] > 0 then
        if Player.Functions.RemoveMoney("bank", Config.PremiumDealerSpots[CurrentSlot]['Price'], "vehicle-shop") then
            ...
            TriggerClientEvent('fw-cardealer2:client:set:stock', -1, NewStock, CurrentSlot)
            TriggerClientEvent('fw-cardealer2:client:spawn:car:premium', src, Config.PremiumDealerSpots[CurrentSlot]['Model'], Plate, NewStock, CurrentSlot)
        else
