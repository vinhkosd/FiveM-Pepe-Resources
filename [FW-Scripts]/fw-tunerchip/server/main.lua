...
Framework.Functions.CreateUseableItem("tunerlaptop", function(source, item)
    ...
    TriggerClientEvent('fw-tunerchip:client:openChip', src)
end)

RegisterServerEvent('fw-tunerchip:server:TuneStatus')
AddEventHandler('fw-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
Framework.Functions.CreateCallback('fw-tunerchip:server:HasChip', function(source, cb)
    local src = source
...
Framework.Functions.CreateCallback('fw-tunerchip:server:GetStatus', function(source, cb, plate)
