...
RegisterNUICallback('save', function(data)
    Framework.Functions.TriggerCallback('fw-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            Framework.Functions.Notify('Tjoenertjip v1.05: Voertuig aangepast!', 'error')

            TriggerServerEvent('fw-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('fw-tunerchip:server:TuneStatus')
AddEventHandler('fw-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    Framework.Functions.TriggerCallback('fw-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    Framework.Functions.Notify('SJPerformance Tunerchip: Voertuig is gereset!', 'error')
end)

RegisterNetEvent('fw-tunerchip:client:openChip')
AddEventHandler('fw-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        Framework.Functions.Progressbar("connect_laptop", "Tunerlaptop wordt aangesloten..", 2000, false, true, {
