...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            Framework.Functions.Progressbar("use_nos", "NOS is connected.", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", Framework.Shared.Items['nitrous'], "remove")
            ...
            Framework.Functions.Notify('Je zit niet in een auto poepje.', 'error')
        end
    else
        Framework.Functions.Notify('Broo de NOS staat toch al aan gekkie..', 'error')
    end
end)
        ...
        if IsInVehicle then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                        ...
                        Citizen.CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    ...
                                    TriggerEvent('fw-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, true)
                                else
                    ...
                    if IsControlJustReleased(0, Keys["LEFTCTRL"]) then
                        if NitrousActivated then
                            ...
                            StopScreenEffect("RaceTurbo")
                            TriggerEvent('fw-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, false)
                            NitrousActivated = false
                        end
                    end
                end
                ...
                if not nosupdated then
                    TriggerEvent('fw-hud:client:UpdateNitrous', false, nil, false)
                    nosupdated = true
                end
            end
        else
