...
RegisterNetEvent('fw-radialmenu:client:flip:vehicle')
AddEventHandler('fw-radialmenu:client:flip:vehicle', function()
    local Vehicle, Distance = Framework.Functions.GetClosestVehicle()
    if Vehicle ~= 0 and Distance < 1.7 then
        Framework.Functions.Progressbar("flip-vehicle", "Voertuig omduwen..", math.random(10000, 15000), false, true, {
...
RegisterNetEvent("fw-radialmenu:client:send:panic:button")
AddEventHandler("fw-radialmenu:client:send:panic:button",function()
  Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
      if HasItem then
          ...
          TriggerServerEvent('fw-police:server:send:alert:panic:button', GetEntityCoords(GetPlayerPed(-1)), StreetLabel, Info)
      else
          Framework.Functions.Notify("Je hebt geen porto..", "error")
      end
  end, "radio")
end)

RegisterNetEvent("fw-radialmenu:client:send:down")
AddEventHandler("fw-radialmenu:client:send:down",function(Type)
    local Player = Framework.Functions.GetPlayerData()
    local Info = {['Firstname'] = Player.charinfo.firstname, ['Lastname'] = Player.charinfo.lastname, ['Callsign'] = Player.metadata['callsign']}
    local StreetLabel = Framework.Functions.GetStreetLabel()
    local Priority = 2
    ...
    TriggerServerEvent('fw-police:server:send:alert:officer:down', GetEntityCoords(GetPlayerPed(-1)), StreetLabel, Info, Priority)
end)

RegisterNetEvent("fw-radialmenu:client:open:door")
AddEventHandler("fw-radialmenu:client:open:door",function(DoorNumber)
...
RegisterNetEvent("fw-radialmenu:client:enter:radio")
AddEventHandler("fw-radialmenu:client:enter:radio",function(RadioNumber)
    Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
        if HasItem then
