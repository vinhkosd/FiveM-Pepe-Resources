$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('http://fw-burgershot/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('http://fw-burgershot/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('http://fw-burgershot/CloseNui', JSON.stringify({}))
        ...
        $.post('http://fw-burgershot/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('http://fw-burgershot/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('http://fw-burgershot/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
