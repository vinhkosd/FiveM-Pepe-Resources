...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(2000, function()
        TriggerServerEvent("fw-clothing:loadPlayerSkin")
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 0.15, '~g~E~w~ - Om kleding te shoppen')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 0.15, '~g~E~w~ - Om haar aan te passen')
                            end
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if Framework.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    Framework.Functions.TriggerCallback('fw-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Presets", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                            {menu = "myOutfits", label = "Trang phục của tôi", selected = false, outfits = result},
                                            {menu = "character", label = "Nhân vật", selected = false},
                                            {menu = "accessoires", label = "Phụ kiện", selected = false}
                                        })
...
RegisterNetEvent('fw-clothing:client:openOutfitMenu')
AddEventHandler('fw-clothing:client:openOutfitMenu', function()
    Framework.Functions.TriggerCallback('fw-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('fw-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('fw-clothing:client:openMenu')
AddEventHandler('fw-clothing:client:openMenu', function()
    customCamLocation = nil
    Framework.Functions.TriggerCallback('fw-clothing:server:getOutfits', function(result)
      openMenu({
          {menu = "character", label = "Nhân vật", selected = true},
          {menu = "clothing", label = "Ngoại hình", selected = false},
          {menu = "myOutfits", label = "Trang phục của tôi", selected = false, outfits = result},
          {menu = "accessoires", label = "Phụ kiện", selected = false}
      })
    end)
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "SYSTEM", "error", "Bạn không thể tháo vòng chân của bạn..")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('fw-clothing:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('fw-clothing:client:reloadOutfits')
AddEventHandler('fw-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('fw-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function SaveSkin()
	...
	TriggerServerEvent("fw-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('fw-clothing:client:CreateFirstCharacter')
AddEventHandler('fw-clothing:client:CreateFirstCharacter', function()
...
RegisterNetEvent("fw-clothing:loadSkin")
AddEventHandler("fw-clothing:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('fw-clothing:client:loadPlayerClothing', data, GetPlayerPed(-1))
    end)
end)

RegisterNetEvent('fw-clothing:client:loadPlayerClothing')
AddEventHandler('fw-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('fw-clothing:client:loadOutfit')
AddEventHandler('fw-clothing:client:loadOutfit', function(oData)
    local ped = GetPlayerPed(-1)
    ...
    if data["accessory"] ~= nil then
        if Framework.Functions.GetPlayerData().metadata["tracker"] then
...
RegisterNetEvent("fw-clothing:client:adjustfacewear")
AddEventHandler("fw-clothing:client:adjustfacewear",function(type)
    if Framework.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
