...
RegisterNetEvent('fw-binds:client:openUI')
AddEventHandler('fw-binds:client:openUI', function()
    openBindingMenu()
end)
        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = Framework.Functions.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('Framework:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            Framework.Functions.Notify('Er is nog niks aan ['..v..'] gebind, /binds om een commando te binden', 'primary', 4000)
                        end
                    else
                        Framework.Functions.Notify('Je hebt nog geen commands gebind, /binds om een commando te binden', 'primary', 4000)
                    end
                end
            end
            if keyPressed then
...
RegisterNUICallback('save', function(data)
    ...
    Framework.Functions.Notify('Commandbindings zijn opgeslagen!', 'success')
    TriggerServerEvent('fw-binds:server:setKeyMeta', keyData)
end)