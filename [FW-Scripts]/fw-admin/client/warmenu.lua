...
local buttonFont = RegisterFontId("arial font")
...
function WarMenu.Button(text, subText)
	local buttonText = "<FONT FACE='arial font'>"..text.."</FONT>"
	if subText then
...
function WarMenu.CheckBox(text, checked, callback)
	if WarMenu.Button(text, checked and '~g~Đã chọn' or '~r~Chưa chọn') then
		checked = not checked
