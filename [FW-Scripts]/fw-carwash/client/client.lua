...
RegisterNetEvent('fw-carwash:client:sync:water')
AddEventHandler('fw-carwash:client:sync:water', function(WaterId)
    StartWashParticle(WaterId)
end)

RegisterNetEvent('fw-carwash:client:stop:water')
AddEventHandler('fw-carwash:client:stop:water', function(WaterId)
...
RegisterNetEvent('fw-carwash:client:set:busy')
AddEventHandler('fw-carwash:client:set:busy', function(CarWashId, bool)
 Config.CarWashLocations[CarWashId]['Busy'] = bool
end)

RegisterNetEvent('fw-carwash:client:sync:wash')
AddEventHandler('fw-carwash:client:sync:wash', function(Vehicle)
                 ...
                 if Gebied <= 1.5 then
                     ...
                     if IsControlJustReleased(0, Config.Keys['G']) then
                        if not Config.CarWashLocations[k]['Busy'] then
                            Framework.Functions.TriggerCallback("fw-carwash:server:can:wash", function(CanWash)
                            if CanWash then
...
function StartCarWashing(CarWashId, Vehicle)
 ...
 TriggerServerEvent('fw-carwash:server:set:busy', CarWashId, true) 
 SetEveryoneIgnorePlayer(PlayerPedId(), true)
 SetPlayerControl(PlayerPedId(), false)
 Wait(250)
 TriggerServerEvent('fw-carwash:server:sync:water', Config.CarWashLocations[CarWashId]['Particle'])
 ...
 TriggerServerEvent('fw-carwash:server:sync:wash', Vehicle)
 Citizen.Wait(Config.CarWashLocations[CarWashId]['Wait'] / 2)
 TriggerServerEvent('fw-carwash:server:stop:water', Config.CarWashLocations[CarWashId]['Particle'])
 SetPlayerControl(PlayerPedId(), true)
 TriggerServerEvent('fw-carwash:server:set:busy', CarWashId, false) 
end
