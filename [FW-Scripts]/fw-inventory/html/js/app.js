...
function FormatAttachmentInfo(data) {
    $.post("http://fw-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('http://fw-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('http://fw-inventory/Notify', JSON.stringify({
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id-card") {
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Betaald: </strong><span>€' + itemData.info.price + '</span></p><p><strong>Notitie: </strong><span>' + itemData.info.note + '</span></p>');
...
function handleDragDrop() {
    ...
    $(".item-slot").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://fw-inventory/UpdateStash", JSON.stringify({}));
        },
    });
    ...
    $("#item-use").droppable({
        ...
        drop: function(event, ui) {
            ...
            if(fromData.useable) {
                ...
                $.post("http://fw-inventory/UseItem", JSON.stringify({
    ...
    $(".item-slot").click(function(event) {
        if (event.shiftKey) {
            ...
            $.post('http://fw-inventory/UseItemShiftClick', JSON.stringify({
                slot: itemData.slot
            }));
        } 
    });
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://fw-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://fw-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://fw-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://fw-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("http://fw-inventory/PlayDropSound", JSON.stringify({}));
            $.post("http://fw-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('http://fw-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("http://fw-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("http://fw-inventory/CloseInventory", JSON.stringify({}));
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('http://fw-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});