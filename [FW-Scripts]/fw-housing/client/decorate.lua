	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z'], false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				Framework.Functions.Notify("Je bent buiten de range gegaan!")
			end
		end
	end
end)
...
RegisterNetEvent("fw-housing:client:decorate")
AddEventHandler("fw-housing:client:decorate", function()
	Citizen.Wait(500)
	if IsInHouse then 
		if HasKey then 
			...
			Framework.Functions.Notify("Je moet de sleutels van het huis hebben!", "error")
		end
	else
		Framework.Functions.Notify("Je bent niet in een huis!", "error")
	end
end)
...
RegisterNetEvent("fw-housing:server:sethousedecorations")
AddEventHandler("fw-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house]['Decorations'] == nil or next(Config.Houses[house]['Decorations']) == nil then
		Framework.Functions.TriggerCallback('fw-housing:server:get:decorations', function(result)
...
function SaveDecorations()
	if Currenthouse ~= nil then
		...
		TriggerServerEvent("fw-housing:server:save:decorations", Currenthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		Framework.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
