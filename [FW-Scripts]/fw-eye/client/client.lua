        ...
        if LoggedIn then
            if InTrunk then
                if IsEntityDead(GetPlayerPed(-1)) then
                    ...
                    TriggerServerEvent('fw-eye:server:set:trunk:data', Plate, false)
                    InTrunk = false
                end
                Citizen.Wait(150)
            else
...
RegisterNetEvent('fw-eye:client:open:trunk')
AddEventHandler('fw-eye:client:open:trunk', function()
...
RegisterNetEvent('fw-eye:client:sync:trunk:data')
AddEventHandler('fw-eye:client:sync:trunk:data', function(ConfigData)
    Config.TrunkData = ConfigData
end)

RegisterNetEvent('fw-eye:client:getin:trunk')
AddEventHandler('fw-eye:client:getin:trunk', function()
    ...
    if Vehicle ~= 0 and Distance < 5.0 then
        ...
        if Config.TrunkData[Plate] == nil then
            TriggerServerEvent('fw-eye:server:setup:trunk:data', Plate)
            Citizen.Wait(150)
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        InTrunk = true
                        exports['fw-assets']:RequestAnimationDict("fin_ext_p1-7")
                        TriggerServerEvent('fw-eye:server:set:trunk:data', Plate, true)
                        ...
                        Framework.Functions.Notify('Dit kan niet eens bij een motor..', 'error', 2500)
                    end
                else
                    Framework.Functions.Notify('Is de kofferbak wel open?', 'error', 2500)
                end
            else
                Framework.Functions.Notify('Er zit al iemand in de kofferbak..', 'error', 2500)
            end
        else
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
        ...
        Framework.Functions.Notify('Geen voertuig gevonden..', 'error', 2500)
    end
end)

RegisterNetEvent('fw-eye:client:getout:trunk')
AddEventHandler('fw-eye:client:getout:trunk', function()
    local Vehicle = GetEntityAttachedTo(PlayerPedId())
    local VehicleCoords = GetOffsetFromEntityInWorldCoords(Vehicle, 0, -5.0, 0)
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
...
RegisterNetEvent('fw-eye:client:refresh')
AddEventHandler('fw-eye:client:refresh', function()
