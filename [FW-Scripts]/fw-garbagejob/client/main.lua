            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        TriggerServerEvent('fw-garbagejob:server:PayShitBakker', Earnings, location)
                        Earnings = 0
                    end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                Framework.Functions.TriggerCallback('fw-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                Framework.Functions.TriggerCallback('fw-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        Framework.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            SetVehicleNumberPlateText(veh, "GARB"..tostring(math.random(1000, 9999)))
                                            SetEntityHeading(veh, coords.h)
                                            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
                                            SetEntityAsMissionEntity(veh, true, true)
                                            exports['fw-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(veh), true)
                                            Citizen.Wait(100)
                                            exports['fw-fuel']:SetFuelLevel(veh, GetVehicleNumberPlateText(veh), 100, false)
